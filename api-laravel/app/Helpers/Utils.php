...
class Utils
{
    ...
    public static function insertMovie($cinemaId, $locationId, $linkCinemaMoviePage, Movie $movieToInsert, $date, $language)
    {
        ...
        if (!$movie) { //if the movie does not exist           
            ...
            if ($movieToInsert->save()) {
                Utils::insertMoviesInCinema($movieToInsert->id, $cinemaId, $locationId, $date, $linkCinemaMoviePage, $language);

                // echo($moviesInCinema);
            }
        } else {
            //if the movie already exists
            Utils::updateMovie($movie, $movieToInsert);
            ...
            if (!$moviesInCinema) {   //if the cinema if not associated with the movie, we create it
                Utils::insertMoviesInCinema($movie->id, $cinemaId, $locationId, $date, $linkCinemaMoviePage, $language);
            }
        }
    }

    public static function insertMoviesInCinema($movieId, $cinemaId, $locationId, $date, $linkCinemaMoviePage, $language)
    {
        return MoviesInCinema::create([
            MoviesInCinema::MOVIE_ID => $movieId,
    ...
    public static function updateMovie($movie, $movieToInsert)
    {
